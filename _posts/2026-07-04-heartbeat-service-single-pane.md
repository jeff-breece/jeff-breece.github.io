...
And it doesn't need to be sophisticated. A script that hits five URLs with a short timeout, prints a table, and stamps the time is most of the value. I ran something very close to that for months before it grew into a service with a UI. The sophistication came later and it came from use - every feature described above exists because I wanted it while staring at the simple version, not because I planned it.
