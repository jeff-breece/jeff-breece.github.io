...
If the answer is "nothing - start another one and it carries on", it belongs in the cluster. It's stateless, it's replaceable, and orchestration is good at managing that kind of thing.
...
The stateless HTTP services - the router, the skill service, the interfaces - are the ones where a pod is better, and those are the ones moving.
...
You don't need Kubernetes. For a homelab, you may never need it - systemd units and a deploy script will run twenty services on one machine perfectly well, and that's most of what this lab did for a year.

If you're curious about it, learn it because you want to, and be honest that you're learning rather than solving. That's a legitimate reason. Just don't let it make you migrate the things that shouldn't move.
