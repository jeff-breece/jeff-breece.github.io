...
The lesson underneath is one I'd generalise hard: **if part of your data has exact answers, do not put that part in a vector store.** Vector search is a wonderful tool for the fuzzy and a poor substitute for a database. A lot of "RAG doesn't work for me" is a question with a crisp answer being asked of an index that can only do resemblance.
