...
Most questions never reach stage two. The ones that do are ambiguous, and paying a couple of seconds on those is fine, because they're the minority and because the alternative is being wrong quickly.
...
The result is confident nonsense sourced from real data, which is a bad failure. It's not obviously wrong. It has the texture of grounded truth, because it *is* built from things I actually wrote - just things that have nothing to do with what I asked.
