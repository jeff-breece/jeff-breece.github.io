...
Alerts have the opposite problem. They interrupt, which means they have to be reserved for things worth interrupting over, which means the vast middle ground - "the lab is working unusually hard right now", "something has been retrying for ten minutes" - has nowhere to live. It's not worth a Slack message. It's interesting.
...
- **Violet pulse** - inference. Something is thinking. On the vision panel it beats faster when the GPU is loaded.
